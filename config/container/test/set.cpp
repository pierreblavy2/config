...
[[maybe_unused]] void set_test(){
	...
	for(int i = 0; i< 10; ++i){
		container::emplace_anywhere(c,i);
	}

	container::remove_front(c);
	container::remove_back(c);
	container::remove_anywhere(c);
	container::remove_here(c,c.begin());
	container::remove_here(c,size_t(1));



	//UNIMPLEMENTED, you're not supposed to edit set's elements
	/*
...
namespace{
[[maybe_unused]] void multiset_test(){
	std::multiset<int> c;
	static_assert( !container::unique_values< decltype(c) >,"" );
	static_assert( container::is_ordered       < decltype(c) >,"" );
