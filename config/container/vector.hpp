...
#ifndef LIB_CONTAINER_VECTOR_HPP_
#define LIB_CONTAINER_VECTOR_HPP_
...
template<typename X_t, typename...X> constexpr bool unique_values< std::vector<X_t, X...> > = false;
template<typename X_t, typename...X> constexpr bool is_ordered < std::vector<X_t, X...> >       = false;

//--- add_xxx ---

//add_front
template<typename X_t, typename...X>
struct Add_front_t< std::vector<X_t, X...> >{
	Add_front_t()=delete;

	typedef std::vector<X_t, X...> container_t;

	template<typename Value_t>
	static void run(container_t &c, Value_t &&v){
		c.insert(c.begin(), std::forward<Value_t>(v) );
	}
...
//add_back
template<typename X_t, typename...X>
struct Add_back_t< std::vector<X_t, X...> >{
	Add_back_t()=delete;
...
//add_anywhere
template<typename X_t, typename ... X>
struct Add_anywhere_t<std::vector<X_t,X...> > : Add_back_t<std::vector<X_t,X...>>{};


//add_here
template<typename X_t, typename ... X>
struct Add_here_t<std::vector<X_t,X...> >{
	Add_here_t()=delete;
	typedef std::vector<X_t,X...> container_t;
...
struct Emplace_front_t< std::vector<Value_t, X...>  > {
	Emplace_front_t()=delete;
	typedef std::vector<Value_t, X...>  container_t;

	template<typename... Args>
	static void run(container_t &c, Args&& ... a){
		c.emplace(c.begin() , std::forward<Args>(a)... );
	}
...
//emplace_back
template<typename Value_t, typename ... X>
struct Emplace_back_t< std::vector<Value_t, X...>  > {
...
//emplace_anywhere
template<typename X_t, typename ... X>
struct Emplace_anywhere_t<std::vector<X_t,X...> > : Emplace_back_t<std::vector<X_t,X...>>{};


//emplace_here
template<typename X_t, typename ... X>
struct Emplace_here_t< std::vector<X_t,X...> >{
...
template<typename X_t, typename ... X>
struct Get_front_t< std::vector<X_t,X...> >{
	Get_front_t()=delete;
	typedef std::vector<X_t,X...> container_t;
...
template<typename X_t, typename ... X>
struct Cget_front_t< std::vector<X_t,X...> >{
	Cget_front_t()=delete;
	typedef std::vector<X_t,X...> container_t;
...
//get_back, cget_back
template<typename X_t, typename ... X>
struct Get_back_t< std::vector<X_t,X...> >{
	Get_back_t()=delete;
	typedef std::vector<X_t,X...> container_t;
...
template<typename X_t, typename ... X>
struct Cget_back_t< std::vector<X_t,X...> >{
	Cget_back_t()=delete;
	typedef std::vector<X_t,X...> container_t;
...
template<typename X_t, typename ... X>
struct Get_anywhere_t<std::vector<X_t,X...> > : Get_back_t<std::vector<X_t,X...>>{};

template<typename X_t, typename ... X>
struct Cget_anywhere_t<std::vector<X_t,X...> > : Cget_back_t<std::vector<X_t,X...>>{};


//get_here, cget_here
template<typename X_t, typename ... X>
struct Get_here_t< std::vector<X_t,X...> >{
	Get_here_t()=delete;
	typedef std::vector<X_t,X...> container_t;
...
template<typename X_t, typename ... X>
struct Cget_here_t< std::vector<X_t,X...> >{
	Cget_here_t()=delete;
	typedef std::vector<X_t,X...> container_t;
...
template<typename X_t, typename ... X>
struct Remove_front_t< std::vector<X_t,X...> >{
...
template<typename X_t, typename ... X>
struct Remove_back_t< std::vector<X_t,X...> >{
...
template<typename X_t, typename ... X>
struct Remove_anywhere_t<std::vector<X_t,X...> > : Remove_back_t<std::vector<X_t,X...>>{};


template<typename X_t, typename ... X>
struct Remove_here_t< std::vector<X_t,X...> >{
...
template<typename X_t, typename ... X>
struct Mem_reserve_t< std::vector<X_t,X...> >{
	Mem_reserve_t()=delete;
	typedef std::vector<X_t,X...> container_t;

	static void run(container_t &c, size_t i){c.reserve(i);}

    static constexpr bool is_implemented   = true;
    static constexpr bool do_something     = true;
    static constexpr Complexity complexity = Complexity::linear;

};

template<typename X_t, typename ... X>
struct Mem_shrink_t< std::vector<X_t,X...> >{
	Mem_shrink_t()=delete;
	typedef std::vector<X_t,X...> container_t;

	static void run(container_t &c){c.shrink_to_fit();}
...
template<typename X_t, typename ... X>
struct Mem_capacity_t< std::vector<X_t,X...> >{
	Mem_capacity_t()=delete;
	typedef std::vector<X_t,X...> container_t;

	static size_t run(const container_t &c){return c.capacity();}
...
//--- count ---
template<typename X_t, typename ... X>
struct Count_empty_t< std::vector<X_t,X...> >{
...
template<typename X_t, typename ... X>
struct Count_all_t< std::vector<X_t,X...> >{
	Count_all_t()=delete;
	typedef std::vector<X_t,X...> container_t;
...
template<typename X_t, typename ... X>
struct Count_element_t< std::vector<X_t,X...> >{
...
#endif /* LIB_CONTAINER_VECTOR_HPP_ */
